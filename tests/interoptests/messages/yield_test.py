...
def is_equal(msg1: messages.Yield, msg2: messages.Yield) -> bool:
    return (
...
async def test_json_serializer():
    ...
    command = f"wampproto message yield {msg.request_id} --serializer json"
...
async def test_with_args_kwargs_options():
    msg = messages.Yield(
        messages.YieldFields(1, options={"a": "b"}, args=["foo"], kwargs={"a": "b"}),
    )
    command = f"wampproto message yield {msg.request_id} foo -o a=b -k a=b --serializer json"
