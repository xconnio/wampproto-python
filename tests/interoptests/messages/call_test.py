...
def is_equal(msg1: messages.Call, msg2: messages.Call) -> bool:
    return (
        ...
        and msg1.procedure == msg2.procedure
...
async def test_json_serializer():
    ...
    command = f"wampproto message call {call_message.request_id} {call_message.procedure} --serializer json"
...
async def test_cbor_serializer():
    call_message = messages.Call(messages.CallFields(1, TEST_PROCEDURE))
    command = (
        f"wampproto message call {call_message.request_id} {call_message.procedure} --serializer cbor --output hex"
    )
...
async def test_msgpack_serializer():
    call_message = messages.Call(messages.CallFields(1, TEST_PROCEDURE))
    command = (
        f"wampproto message call {call_message.request_id} {call_message.procedure} --serializer msgpack --output hex"
    )
...
async def test_with_args_kwargs():
    ...
    command = (
        f"wampproto message call {call_message.request_id} {call_message.procedure} abc 123 -k a=b --serializer json"
    )
