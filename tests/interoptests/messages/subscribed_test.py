...
def is_equal(msg1: messages.Subscribed, msg2: messages.Subscribed) -> bool:
    return msg1.request_id == msg2.request_id and msg1.subscription_id == msg2.subscription_id


@pytest.mark.asyncio
async def test_json_serializer():
    msg = messages.Subscribed(messages.SubscribedFields(1, 1))
    command = f"wampproto message subscribed {msg.request_id} {msg.subscription_id} --serializer json"
...
async def test_cbor_serializer():
    msg = messages.Subscribed(messages.SubscribedFields(1, 1))
    command = f"wampproto message subscribed {msg.request_id} {msg.subscription_id} --serializer cbor --output hex"
...
async def test_msgpack_serializer():
    msg = messages.Subscribed(messages.SubscribedFields(1, 1))
    command = f"wampproto message subscribed {msg.request_id} {msg.subscription_id} --serializer msgpack --output hex"
