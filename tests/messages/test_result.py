...
def test_parse_with_invalid_list_length():
    ...
    assert (
        str(exc_info.value)
        == f"invalid message length: expected length 'between 3 & 5', got '{len(message)}' for '{messages.Result.RESULT_TEXT}'"
    )
...
def test_parse_with_invalid_args_type():
    ...
    assert (
        str(exc_info.value) == f"invalid type: expected type 'list', got 'str' for args in '{messages.Result.RESULT_TEXT}'"
    )
