...
class Welcome(Message):
    ...
    def parse(msg: list[Any]) -> Welcome:
        ...
        if not isinstance(roles, dict):
            raise exception.ProtocolError(f"invalid type for 'roles' in details for {Welcome.TEXT}")

        if len(roles) == 0:
            raise exception.ProtocolError(f"roles are missing in details for {Welcome.TEXT}")

        # for role in roles.keys():
        #     if role not in util.AllowedRoles.__members__.values():
        #         raise exception.ProtocolError(f"invalid role '{role}' in 'roles' details for {Welcome.WELCOME_TEXT}")

        authid = details.get("authid", None)
        if authid is not None:
            if not isinstance(authid, str):
                raise exception.ProtocolError(f"authid must be a type string for {Welcome.TEXT}")

        authrole = details.get("authrole", None)
        if authrole is not None:
            if not isinstance(authrole, str):
                raise exception.ProtocolError(f"authrole must be a type string for {Welcome.TEXT}")

        authmethod = details.get("authmethod", None)
        if authmethod is not None:
            if not isinstance(authmethod, str):
                raise exception.InvalidTypeError(str, type(authmethod), "authmethod", Welcome.TEXT)

        authextra = details.get("authextra", None)
        if authextra is not None:
            if not isinstance(authextra, dict):
                raise exception.InvalidTypeError(dict, type(authextra), "authextra", Welcome.TEXT)
